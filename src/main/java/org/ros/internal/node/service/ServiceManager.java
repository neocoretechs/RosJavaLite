...
public class ServiceManager {
  ...
  public ServiceManager() {
    serviceServers = new ConcurrentHashMap<GraphName, DefaultServiceServer<?, ?>>();
    serviceClients = new ConcurrentHashMap<GraphName, DefaultServiceClient<?, ?>>();
  }
  ...
  public List<DefaultServiceServer<?, ?>> getServers() {
	  Collection<DefaultServiceServer<?, ?>> cdss = serviceServers.values();
	  ArrayList<DefaultServiceServer<?, ?>> al = new ArrayList<DefaultServiceServer<?, ?>>(cdss.size());
	  for(DefaultServiceServer<?, ?> dss : cdss)
		  al.add(dss);
   //return (List<DefaultServiceServer<?, ?>>)serviceServers.values();
	  return al;
  }

  public List<DefaultServiceClient<?, ?>> getClients() {
	  Collection<DefaultServiceClient<?, ?>> cdsc = serviceClients.values();
	  ArrayList<DefaultServiceClient<?, ?>> al = new ArrayList<DefaultServiceClient<?, ?>>(cdsc.size());
	  for(DefaultServiceClient<?, ?> dsc : cdsc)
		  al.add(dsc);
	  return al;
    //return (List<DefaultServiceClient<?, ?>>)serviceClients.values();
  }
}
